...
_BUYBACK_TOP_N = 30
_BUYBACK_RT_COST = 0.0020        # 20bps blended round-trip on the fraction turned over
_BUYBACK_TIERS = ("small", "mid", "large")
_BUYBACK_ETF = "PKW"            # Invesco BuyBack Achievers (share-count-reduction index)


def _run_buyback(conn, study: dict) -> list[dict]:
    """Monthly-rebalance NET-REPURCHASE-YIELD backtest: long the top-N genuine
    net repurchasers (buyback yield = -ncfcommon/marketcap > 0) vs the equal-weight
    PIT universe AND the PKW buyback-achievers ETF. Verdict = gates.lt_factor_verdict
    — it must beat BOTH benchmarks (the 'why not just buy the ETF' honesty bar, the
    same one lt_factor was held to). See studies/buyback.md.

    Structure mirrors _run_lt_factor (monthly PIT universe, closeadj total-return
    panel, turnover cost on the portfolio leg only, IS/OOS clustered active-return t,
    BACKTEST compounded curve) — inlined rather than shared so the promoted lt_factor
    path is untouched."""
    from datetime import datetime, timezone
    ...
    lake = Lake(cfg.data_lake_path)
    prices = _monthly_price_panel(lake, "sep")
    etf = _monthly_price_panel(lake, "sfp", tickers=(_BUYBACK_ETF,))
    med = _month_end_dates(lake)
    art = lake.query(
        f"SELECT ticker, datekey, ncfcommon, marketcap FROM {lake.sql_table('sf1')} "
        f"WHERE dimension = 'ART'").sort_values("datekey")

    months = [m for m in prices.index if m in etf.index]
    # need a next month for the forward return -> rebalance range [1, len-2]
    rebal = months[1:-1]
    print(f"buyback: {len(rebal)} monthly rebalances {rebal[0]}..{rebal[-1]}")

    def etf_ret(i):
        try:
            return etf.loc[months[i + 1], _BUYBACK_ETF] / etf.loc[months[i], _BUYBACK_ETF] - 1
        except (KeyError, ZeroDivisionError):
            return None
    ...
    for ym in rebal:
        ...
        as_of = med[ym]
        universe = {r["ticker"] for r in eq_universe.build_from_lake(lake, as_of)
                    if r["included"] and r["tier"] in _BUYBACK_TIERS}
        if not universe:
            port_ret.append(None); uni_ret.append(None); e_ret.append(None)
            month_ts.append(None); continue
        # PIT fundamentals: latest ART datekey <= as_of, per ticker
        fund = art[art["datekey"] <= as_of].groupby("ticker").tail(1)
        fund = fund[fund["ticker"].isin(universe)].copy()
        fund = fund[(fund["marketcap"] > 0) & fund["ncfcommon"].notna()]
        # buyback yield = net $ returned via repurchases / market cap; > 0 = a real
        # net repurchaser (ncfcommon is negative when the firm buys back stock)
        fund["bby"] = -fund["ncfcommon"] / fund["marketcap"]
        sel_tickers = list(fund[fund["bby"] > 0].nlargest(_BUYBACK_TOP_N, "bby")["ticker"])
        if not sel_tickers:
            port_ret.append(None); uni_ret.append(None); e_ret.append(None)
            month_ts.append(None); continue
        ...
        turnover = (len(cur - prev_sel) / len(cur)) if cur else 0.0
        prev_sel = cur
        port_ret.append(sum(sel_fwd) / len(sel_fwd) - turnover * _BUYBACK_RT_COST)
        uni_ret.append(sum(uni_fwd) / len(uni_fwd))
        e_ret.append(etf_ret(i))
        month_ts.append(int(datetime.strptime(as_of, "%Y-%m-%d")
                            .replace(tzinfo=timezone.utc).timestamp() * 1000))

    # segment by IS <= 2021-12-31 / OOS after (identical to lt_factor)
    def seg(ts): return "IS" if ts <= walkforward.IS_END_MS else "OOS"
    rows_active_u: dict = {"IS": [], "OOS": []}
    rows_active_e: dict = {"IS": [], "OOS": []}
    ts_by: dict = {"IS": [], "OOS": []}
    ...
    sha, now = emitter_sha(), _now_ms()
    p_hash = params_hash({"top_n": _BUYBACK_TOP_N, "rt_cost": _BUYBACK_RT_COST,
                          "tiers": _BUYBACK_TIERS, "etf": _BUYBACK_ETF,
                          "signal": "neg_ncfcommon_over_mktcap"})
    out_rows, legs = [], {}
    for segment in ("IS", "OOS"):
        au, ts_u = rows_active_u[segment], ts_by[segment]
        ae = rows_active_e[segment]
        ...
        t_e = (stats.clustered_t([a for a, _ in ae], [t for _, t in ae])["t"]
               if len(ae) >= 2 else None)
        legs[segment] = {"t_vs_universe": t_u, "t_vs_etf": t_e, "n_months": len(au)}
        out_rows.append({
            "study": study["name"], "segment": segment, "horizon": 0,
            "n_events": len(au), "n_months": len(au),
            "mean_car": sum(au) / len(au),
