...
METHOD = ("cross-sectional replay of the live selection (rank -> expectancy-weighted "
          "priority -> top-N -> cooldown), next-bar-open fills, net of costs; PEAD "
          "aligned to announcement dates (quarter-end-dominated maps rejected); "
          ...
          "delta_* fields vs that baseline")
...
def run_backtest(bars_by_ticker: dict[str, list[dict]],
    ...
    committed winrates, to avoid circularity), top-``stock_top_n`` cut, then the
    live cooldown + no-open-duplicate gates. Surfaced setups fill at the NEXT
    bar's open. Returns closed trades (context fixed at 0 — no historical
    insider/short-vol/revision data exists offline)."""
    ...
    for date in all_dates:
        ...
        for tk, bars in bars_by_ticker.items():
            ...
            earn = None
            emap = earnings_by_ticker.get(tk)
            if emap:
                lo = feat["last_ts"] - cfg.stock_pead_lookback_days * DAY_MS
                cands = [e for ts, e in emap.items() if lo <= ts <= feat["last_ts"]]
...
def baseline_trades(bars_by_ticker: dict[str, list[dict]], cfg, seed: int = 42) -> list[dict]:
    ...
    costs). Each sampled date opens one BUY per archetype frame so every archetype
    cell has a matched baseline in its own frame."""
...
def build_cells(trades: list[dict], baseline: list[dict] | None = None) -> dict:
    ...
    for k, v in summary["archetypes"].items():
        ...
        for t in by_arch.get(k, []):
            m = t.get("month")
            if m:
                months.setdefault(m, []).append(float(t.get("realized_r") or 0.0))
        month_means = [sum(rs) / len(rs) for rs in months.values()]
        ...
        # Pre-registered significance rule (month-clustered): the expectancy delta
        # over the random-entry control must clear 2 SE of the per-month mean-R
        # dispersion across >= 6 distinct months, with a positive win-rate delta.
        # Cells without a control (the live promotion path) stay not_significant
        # until a baselined backtest regen clears them.
        significant = False
        if (baseline is not None and b and b["n"] and len(month_means) >= 6
                and cell["expectancy_r_month_std"]):
            se = cell["expectancy_r_month_std"] / (len(month_means) ** 0.5)
            significant = (cell["delta_expectancy_r"] > 2 * se
                           and cell["delta_win_rate"] > 0)
...
def main(argv=None) -> int:
    ...
    winrates = {"generated_at": datetime.now(timezone.utc).isoformat(),
                ...
                "note": ("In-sample cross-sectional seed (survivorship-biased); measures the "
                         "SURFACED population (rank -> priority -> top-N -> cooldown), context "
                         "fixed at 0."),
                "method": METHOD,
                "baseline": base_cells,
                "archetypes": cells}
